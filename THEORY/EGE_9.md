...
#### Код #2 открытия файла:

``` python

for s in open('название файла'):
    a = (list(map(int, s.split("(тот разделитель, который мы выбрали) например ';'"))))

остальной код...
```

### Полезные функции которые стоит запомнить

#### #1:
Для проверки построчно:

``` python
print(a)
input()  #стопорит код(пауза)
```
#### #2:
...
узнаем кол-во уникальных элементов(удаляем из списка все повторяющиеся)
``` python
len(set(a))==5 #
```
#### #4:

узнаем есть ли в числе 3 повторяющихся числа
``` python
for i in (a):
	if a.count(i)==3:
```
